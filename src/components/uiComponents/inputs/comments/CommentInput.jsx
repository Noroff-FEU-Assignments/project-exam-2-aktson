...
function CommentInput({ showCommentInput, id, setShowCommentInput, setComments }) {
	...
	const {
		...
		reset,
		formState: { errors },
	} = useForm({ resolver: yupResolver(commentsSchema) });
			...
			{showCommentInput && (
				<Animate>
					<fieldset className="flex w-full items-center bg-lightGray p-4 rounded-xl" disabled={isSubmitting}>
						...
						<Button onClick={handleSubmit(postComment)} size="sm" variant="text" className=" flex gap-1 items-center text-primary">
