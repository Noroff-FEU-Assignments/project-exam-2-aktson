    ...
    return (
        ...
        <form className='w-full max-w-sm hidden sm:block relative' >
            <Input variant="standard" ref={inputRef}
                ...
                icon={!show ?
                    <MdPersonSearch size={26} className=" -translate-y-2  -translate-x-4" />
                    : <MdClear size={26} className=" -translate-y-2  -translate-x-4 cursor-pointer" onClick={handleClose} />}
                value={value}
                ...
                <div className='absolute grid grid-cols-2 bg-lightGray rounded-xl overflow-x-hidden  styledScrollbar max-h-72 mt-1 shadow-xl'>
                    {filteredUsers &&
                        ...
                        filteredUsers.map(user => {
                            return <div key={user.name} className="flex gap-2 bg-gray-200 p-1 rounded-xl shadow-md overflow-hidden m-2 items-center">
